...
from models.NCDM_add_video import NCDM
from models.DKVMN_add_video import DKVMN
...
def CDBD_data_helper(args,logger):
    ...
    # same way build user2video
    # in file preproces

    file = open('user2video.txt', 'r') 
    js = file.read()
    user2video = json.loads(js)   
    file.close()  

    raw_video = set(user2video.values())
    num_videos = len(raw_video)
    map_videos = {c:i+1 for i, c in enumerate(raw_video)}
    logger.info("number of video: %d" % num_videos)
    args.num_cognitive = num_videos

    pdb.set_trace()
        
    # build user_id ->video_id
    userid2videoid = {}
    for key, value in tqdm.tqdm(user2video.items()):
        userid2videoid[map_users[key]]=map_videos[value]
    pdb.set_trace()
    # saved
    item_df = pd.DataFrame({'user_id':userid2videoid.keys(),'last_video':userid2videoid.values()})
    item_df.to_csv(args.saved_cog_dir,sep=',',index=False, header=True)
    logger.info("video.csv saved!")
...
def transform1(user, item, item2knowledge, score, batch_size,knowledge_n, item2cognitive, cognitive_n):
    ...
    for idx in range(len(user)):
        cognitive_emb[idx][np.array(item2cognitive[user[idx]]) - 1] = 1.0
...
def CDBD_Baseline(args,logger):
    ...
    for i, s in df_cog.iterrows():
        use_id, video = s['user_id'], s['last_video']
        user2video[use_id] = video
        video_set.add(video)
    ...
    video_n = np.max(list(video_set))
    logger.info('user_n: {}\n item_n: {}\n knowledge_n: {}\n video_n: {}\n'.format(user_n,item_n,knowledge_n,video_n))
    
    pdb.set_trace()
    
    if args.models == 'NCDM':
        train_set, valid_set, test_set = [
            transform1(data["user_id"], data["problem_id"], item2knowledge, data["is_correct"], args.batch_size, knowledge_n, user2video, video_n)
            for data in [train_data, dev_data, test_data]
        ]  
        cdm = NCDM(knowledge_n, item_n, user_n, video_n)
    elif args.models == 'MIRT' or args.models == 'GDIRT':
        train_set, valid_set, test_set= [
            transform2(data["user_id"], data["problem_id"], data["is_correct"],args.batch_size,user2video, video_n)
            for data in [train_data, dev_data, test_data]
        ]
        # if args.models == 'GDIRT':
        #     cdm = IRT(user_n+1,item_n+1, cognitive_n)
...
def DKVMN_Baseline(args, logger):
    ...
    params = {
        ...
        'seqlen': args.seqlen, # 'the allowed maximum length of a sequence'
        'data_dir': '../../data/DKT', # 'data directory'
