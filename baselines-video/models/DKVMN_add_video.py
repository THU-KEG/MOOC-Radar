...
class Net(nn.Module):
    def __init__(self, n_question, batch_size, key_embedding_dim, value_embedding_dim,
                 memory_size, key_memory_state_dim, value_memory_state_dim, final_fc_dim, n_pid, student_num=None):
        ...
        # add cognitive
        if self.n_pid > 0:
            # self.pid_embed_q = nn.Embedding(self.n_pid+1,int(self.key_embedding_dim/2),padding_idx = 0 )
            # self.pid_embed_qa = nn.Embedding(self.n_pid+1,int(self.value_embedding_dim/2),padding_idx=0)

            # # init new params
            # nn.init.kaiming_normal_(self.pid_embed_q.weight,0.)
            # nn.init.kaiming_normal_(self.pid_embed_qa.weight,0.) 

            self.difficult_param1 = nn.Embedding(50, 1)

            # 50 is sequence lengths
            self.pid_onehot_embedding = nn.Linear(self.n_pid+1,50,bias=True)
            nn.init.constant_(self.pid_onehot_embedding.bias, 0)
            
            self.difficult_param2 = nn.Linear(512,256,bias=True)
            self.difficult_param3 = nn.Linear(256,1,bias=True)
            self.drop_1 = nn.Dropout(p=0.5)
            self.drop_2 = nn.Dropout(p=0.5)
            nn.init.constant_(self.difficult_param2.bias, 0)
            nn.init.constant_(self.difficult_param3.bias, 0)


            self.q_embed_diff = nn.Embedding(self.n_question + 1, self.key_embedding_dim)
            self.qa_embed_diff = nn.Embedding(2 * self.n_question + 1, self.value_embedding_dim)
            
            # init new params
            nn.init.constant_(self.difficult_param1.weight,0.)
            nn.init.kaiming_normal_(self.q_embed_diff.weight)
            nn.init.kaiming_normal_(self.qa_embed_diff.weight)
    ...
    def forward(self, q_data, qa_data, target,pid_data = None):
        # q_data is problem 
        # pid_data is cognitive dimension
        ...
        # add cognitive to q_embed_data


        if self.n_pid > 0:
            # q_embed_data= torch.cat([q_embed_data, self.pid_embed_q(pid_data)],dim=2)
            # qa_embed_data= torch.cat([qa_embed_data, self.pid_embed_qa(pid_data)],dim=2)
 
            q_embed_diff_data = self.q_embed_diff(q_data)
            

            # pid_data shape: (batch_size,seqlen)->(batch_size,1)
            # import pdb;pdb.set_trace()
            pid_data_emb = torch.zeros(batch_size,self.n_pid+1)
            for idx in range(len(pid_data)):
                last_video = 0
                for each_video in pid_data[idx]:
                    # last video same
                    if last_video == each_video:
                        break
                    last_video = each_video
                    pid_data_emb[idx][each_video] = 1.0
                    # only keep one
                    break
            # from (batch_size, pid_one_hot)->(batch_size, seqlen)
            
            pid_data_emb = torch.FloatTensor(pid_data_emb).to(device)
            pid_data_emb = self.pid_onehot_embedding(pid_data_emb)

            # pid_embed_data = self.drop_1(torch.sigmoid(self.difficult_param1(pid_data_emb.long())))
            # pid_embed_data = self.drop_2(torch.sigmoid(self.difficult_param2(pid_embed_data)))
            # pid_embed_data = torch.sigmoid(self.difficult_param3(pid_embed_data))
            
            pid_embed_data = self.difficult_param1(pid_data_emb.long())
...
class DKVMN(KTM):
    def __init__(self, n_question, batch_size, key_embedding_dim, value_embedding_dim,
        ...
        self.model = Net(n_question, batch_size, key_embedding_dim, value_embedding_dim,
                         memory_size, key_memory_state_dim, value_memory_state_dim, final_fc_dim,n_pid, student_num)

    def train_epoch(self, epoch, model, params, optimizer, q_data, qa_data, pid_data):
        ...
        for idx in tqdm(range(N), "Epoch %s" % epoch):
            ...
            input_pid = torch.LongTensor(pid_one_seq).to(device)
            
            model.zero_grad()
            loss, filtered_pred, filtered_target = model.forward(input_q, input_qa, target_1d, input_pid)
            loss.backward()
        ...
        return epoch_loss / N, accuracy, auc, all_loss

    def train(self, params, train_data, test_data=None):
        pid_data, qa_data, q_data = train_data
        ...
        for idx in range(params['max_iter']):
            train_loss, train_accuracy, train_auc, all_loss_list = self.train_epoch(idx, model, params, optimizer, q_data, qa_data, pid_data)
            
            for batch_count in range(len(all_loss_list)):
                df_step.loc[len(df_step.index)]=[batch_count + idx * params['batch_size'], all_loss_list[batch_count]]            
            ...
            if test_data is not None:
                ...
                df_epoch.loc[len(df_epoch.index)]=[idx, valid_auc, valid_accuracy]
        


    def eval(self, params, data):
        pid_data, qa_data, q_data = data
        model = self.model
        N = int(math.floor(len(q_data) / params['batch_size']))
