...
from EduKTM import DKT, AKT, DKTPlus
from EduCDM import MIRT

from load_data import DATA, PID_DATA


from models.NCDM_add_cognitive import NCDM
from models.DKVMN_add_cognitive import DKVMN
...
def CDBD_Baseline(args,logger):
    ...
    logger.info('user_n: {}\n item_n: {}\n knowledge_n: {}\n cognitive_n: {}\n'.format(user_n,item_n,knowledge_n,cognitive_n))
    
    pdb.set_trace()
    
    if args.models == 'NCDM':
        train_set, valid_set, test_set = [
            transform1(data["user_id"], data["problem_id"], item2knowledge, data["is_correct"], args.batch_size, knowledge_n, item2cognitive, cognitive_n)
            for data in [train_data, dev_data, test_data]
        ]  
        cdm = NCDM(knowledge_n, item_n, user_n, cognitive_n)
    elif args.models == 'MIRT' or args.models == 'GDIRT':
        train_set, valid_set, test_set= [
            transform2(data["user_id"], data["problem_id"], data["is_correct"],args.batch_size,item2cognitive, cognitive_n)
            for data in [train_data, dev_data, test_data]
        ]
